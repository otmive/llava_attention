...
def plot_all_layers(model_name, data_dir):
    ...
    for pos in ['left', 'right', 'neg_left', 'neg_right']:
        ...
        for img_path in os.listdir(data_dir)[0:5]:
            ...
            if pos == 'left':
                ...
                bbox_attentions, baseline_attentions = plotter.plot_attention_through_layers(save_path=f"bar_plots/test_{pos}.png")
                mentioned_attn.append(np.mean(bbox_attentions[left_colour]))
                not_mentioned_attn.append(np.mean(bbox_attentions[right_colour]))
                baseline_attn.append(np.mean(baseline_attentions))
                total_attn.append(plotter.get_total_image_attention())
                calc_total_attn.append(np.mean(bbox_attentions[right_colour]) + np.mean(bbox_attentions[left_colour]) + np.mean(baseline_attentions))
                shape_attn.append(np.mean(bbox_attentions[left_colour]) + np.mean(bbox_attentions[right_colour]))
            elif pos == 'right':
                plotter.get_outputs(f"The figure {right_colour}")
                bbox_attentions, baseline_attentions = plotter.plot_attention_through_layers(save_path=f"bar_plots/test_{pos}.png")
                mentioned_attn.append(np.mean(bbox_attentions[right_colour]))
                not_mentioned_attn.append(np.mean(bbox_attentions[left_colour]))
                baseline_attn.append(np.mean(baseline_attentions))    
                total_attn.append(plotter.get_total_image_attention())
                calc_total_attn.append(np.mean(bbox_attentions[right_colour]) + np.mean(bbox_attentions[left_colour]) + np.mean(baseline_attentions))
                shape_attn.append(np.mean(bbox_attentions[left_colour]) + np.mean(bbox_attentions[right_colour]))
                ...
                bbox_attentions, baseline_attentions = plotter.plot_attention_through_layers(save_path=f"bar_plots/test_{pos}.png")
                mentioned_attn_neg.append(np.mean(bbox_attentions[left_colour]))
                not_mentioned_attn_neg.append(np.mean(bbox_attentions[right_colour]))
                baseline_attn.append(np.mean(baseline_attentions))
                total_attn.append(plotter.get_total_image_attention())
                calc_total_attn.append(np.mean(bbox_attentions[right_colour]) + np.mean(bbox_attentions[left_colour]) + np.mean(baseline_attentions))
                shape_attn.append(np.mean(bbox_attentions[left_colour]) + np.mean(bbox_attentions[right_colour]))
                ...
                bbox_attentions, baseline_attentions = plotter.plot_attention_through_layers(save_path=f"bar_plots/test_{pos}.png")
                mentioned_attn_neg.append(np.mean(bbox_attentions[right_colour]))
                not_mentioned_attn_neg.append(np.mean(bbox_attentions[left_colour]))
                baseline_attn.append(np.mean(baseline_attentions))
                total_attn.append(plotter.get_total_image_attention())
                calc_total_attn.append(np.mean(bbox_attentions[right_colour]) + np.mean(bbox_attentions[left_colour]) + np.mean(baseline_attentions))
                shape_attn.append(np.mean(bbox_attentions[left_colour]) + np.mean(bbox_attentions[right_colour]))
    ...
    print("Average mentioned attention (positive prompt): ", np.mean(mentioned_attn))
    print("Average not mentioned attention (positive prompt): ", np.mean(not_mentioned_attn))
    print("Average mentioned attention (negative prompt): ", np.mean(mentioned_attn_neg))
    print("Average not mentioned attention (negative prompt): ", np.mean(not_mentioned_attn_neg))
    print("Average baseline attention: ", np.mean(baseline_attn))
    print("Average total image attention: ", np.mean(total_attn))
    print("Average calculated total attention: ", np.mean(calc_total_attn))
    print("Average shape attention: ", np.mean(shape_attn))

    final_data = {
        'pos_means': [np.mean(mentioned_attn), np.mean(not_mentioned_attn)], # mentioned, not mentioned
        'pos_sems': [sem_mentioned, sem_not_mentioned],
        'neg_means': [np.mean(mentioned_attn_neg), np.mean(not_mentioned_attn_neg)], # mentioned, not mentioned
        'neg_sems': [sem_mentioned_neg, sem_not_mentioned_neg]
    }
...
def plot_all_layers_4(model_name, data_dir):
    ...
    for pos in base_positions:
        ...
        for is_negative in [False, True]:
            ...
            for img_path in image_files[0:100]: # Adjust range as needed
                try:
                    ...
                    bbox_attentions, _ = plotter.plot_attention_through_layers("tmp_plot")

                    # Layer Selection Logic
                    if 'llava' in model_name.lower(): key_layers = list(range(15, 27))
                    elif 'internvl' in model_name.lower(): key_layers = list(range(20, 35))
                    elif 'paligemma' in model_name.lower(): key_layers = list(range(11, 24))
                    else: key_layers = [0]
                    ...
                    for p_key in base_positions:
                        ...
                        if color in bbox_attentions:
                            ...
                            avg_attn = np.mean(attn_list[key_layers])
    ...
    final_data = {
        ...
        'pos_sems': [sem_mentioned_pos, sem_not_mentioned_pos],
...
if __name__ == "__main__":
    plot_all_layers("internvl", "2d_dataset_fixed_positions_1000/images")
...
#         'neg_not_mentioned': '#EF5350'   # Light red
#     }
...
#             sems = data['pos_sems'] + data['neg_sems']
...
#         ax.set_ylabel('Attention', fontsize=18, fontweight='600', labelpad=12, color='black')
...
#     def create_professional_plot(master_data, models, output_path="plots/2d_datasets_2/point_plots_test.png"):
#         """
...
#                 means = data['pos_means'] + data['neg_means']
#                 sems = data['pos_sems'] + data['neg_sems']
#                 max_val = max(max_val, max([m + s for m, s in zip(means, sems)]))
...
# # fig = create_professional_plot(master_data, models)