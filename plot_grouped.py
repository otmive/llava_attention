...
def main(colour):
    ...
    count = 0
    for i in range(1000):
        image_data = "2d_dataset_fixed_positions_1000/labels.json"
        with open(image_data, 'r') as f:
            data = json.load(f)
        image_url = f"2d_dataset_fixed_positions_1000/images/image_{i:04d}.png"
    ...
    np.save(f"attention_saves/right_not_{colour}.npy", first_item_attention)
    np.save(f"attention_saves/right_not_{colour}_second.npy", second_item_attention)
    np.save(f"attention_saves/right_not_{colour}_base.npy", base_attention)
    ...
    plt.ylim(0, 0.007)
    plt.plot(range(32), avg_first_item_attention, label="Average Left Item Attention", marker='o', color='#800080')
    plt.plot(range(32), avg_second_item_attention, label="Average Right Item Attention", marker='o', color=colour if colour != "yellow" else "#FFD700" )
    ...
    plt.title(f"Average Attention to {colour.capitalize()} Shape (Right Position Negated)")
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"all_1000_right_not_{colour}.png")
    print("count", count)
